...
var getFeetPerMile = require('./Library/GetFeetPerMile');
...
app.get('/getFeetPerMile', function(request, response) {
	console.log('getFeetPerMile called');
	response.send({ "result": getFeetPerMile.getFeetPerMile() });
});

// With a get, the parameters are passed in request.query
app.get('/computeFeetPerMile', function(request, response) {
	console.log('computeFeetPerMile called');	
	console.log(request.query);	
	var operandA = parseInt(request.query.operandA);
	var operandB = parseInt(request.query.operandB);
	var result = addingMachine.myObject.computeFeetPerMile(operandA, operandB);
...
app.post('/GetCircumference', function(request, response) {
	...
	var operandA = parseInt(request.body.operandA);
	//var operandB = parseInt(request.body.operandB);
	var result = addingMachine.myObject.getCircumference(operandA);
	response.send({ "result": result });
});
