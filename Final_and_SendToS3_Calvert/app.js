...
app.get('/saveOptions', function(request, response) {'use strict';
...
app.get('/insertOptions', function(request, response) { 'use strict';
	//message("Server side request for Options");
	var fileContent = fs.readFileSync('Options.json', 'utf8');
	queryMongo.insertIntoCollection(response, JSON.parse(fileContent));
});

app.get('/insertMTC', function(request, response) { 'use strict';
	//message("Server side request for Options");
